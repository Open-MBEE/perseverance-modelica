...
model Machine "Electric machine with MutliBody3D support"
  extends .Electrification.Machines.Templates.Machine3D(
    ...
    redeclare replaceable .Electrification.Machines.Core.Examples.Ideal core(limits(enable_events = false)),
    redeclare replaceable .Electrification.Machines.Control.MultiMode controller(mode = Electrification.Utilities.Types.MachineControlMode.Speed,external_speed = true),fixed_temperature = true,multibody3D(visualize_shape = false),enable_mount = false,visualize = false);
